...
2. Design + declare target collections so each field shows a real control (relations `hasMany`, `fields.color()`, `fields.link()`, `fields.image()`, typed `repeater` with `itemFields`, `slug({ unique:false })`, `translatable: true` on every translated field, `_sourceLocale` on single-language docs). `pnpm cms:generate && pnpm cms:push` (`RECREATE=slug pnpm cms:push --allow-data-loss` for renames).
